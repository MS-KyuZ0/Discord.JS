...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("help")
    .setDescription("Show all command."),
  /**
  ...
  async execute(interaction, client) {
    ...
    const isDir = [
      ...new Set(interaction.client.slashCommand.map((cmd) => cmd.folder)),
    ];
    const formatString = (str) =>
      `${str[0].toUpperCase()}${str.slice(1).toLowerCase()}`;
    const categories = isDir.map((dir) => {
      const getCommands = interaction.client.slashCommand
    ...
    isCollectors.on("collect", (interaction) => {
